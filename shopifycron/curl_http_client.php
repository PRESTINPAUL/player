...
class Curl_HTTP_Client {
	 ...
	 * @access public
	 */
	function send_post_data($url, $postdata, $ip = null, $timeout = 10, $method = 'POST') {
    ...
    /* for updating inventory using new apis */
	function update_inventory_curl($url, $postdata, $ip = null, $timeout = 10, $method = 'POST'){
        curl_setopt($this->ch, CURLOPT_URL, $url);
		curl_setopt($this->ch, CURLOPT_CUSTOMREQUEST, "POST");
		curl_setopt($this->ch, CURLOPT_POSTFIELDS, $postdata);
		curl_setopt($this->ch, CURLOPT_RETURNTRANSFER, true);
		curl_setopt($this->ch, CURLOPT_HTTPHEADER, array( 'Content-Type: application/json', 'Content-Length: ' . strlen($postdata)) );
		$result = curl_exec($this->ch);
		if (curl_errno($this->ch)) {
			if ($this->debug) {
				echo "Error Occured in Curl\n";
